...
class SimpleSite extends SimpleDisplay
{
	public function simpleLoader($name)
	{
		if(@($_GET['debug'])==1)
		{
			echo "Dbg: Attempting to autoload $name...".time();
		}
		if((in_array($name,$this->mods)) && !(in_array($name,$this->loaded)))
		{
			...
			{
				if(@($_GET['debug']==1)) echo "Error!".time()."\n";
			}
			else
			{
				$this->loaded[]=$name;
				if(@($_GET['debug'])==1) echo "Good.".time()."\n";
			}
		}
	}
	...
	{
		if(@($_GET['debug'])==1)
			echo "Dbg: SimpleSite->__call($method,\$args)".time()."\n";
	...
	{
		if(@($_GET['debug'])==1)
			echo "Dbg: SimpleSite->__construct()".time()."\n";
